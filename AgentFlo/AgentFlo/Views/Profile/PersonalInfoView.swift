...
struct PersonalInfoView: View {
    ...
    private func loadAvatarImage() async {
        ...
        if let cached = await ImageCache.shared.image(for: avatarUrl) {
        ...
        do {
            ...
            if let uiImage = UIImage(data: data) {
                await ImageCache.shared.store(uiImage, for: avatarUrl)
    ...
    private func saveProfile() async {
        ...
        do {
            ...
            await appState.authService.fetchUserProfile(userId: user!.id, forceRefresh: true)
            showSuccess = true
        } catch {
