...
struct TaskCreationSheet: View {
            ...
            VStack(alignment: .leading, spacing: Spacing.xxl) {
                ...
                ForEach(TaskCategory.allCases, id: \.self) { cat in
                    Button {
                        ...
                        path.append(cat.displayName)
                    } label: {
                        HStack(spacing: Spacing.xxl) {
                            CategoryIcon(category: cat.displayName)
    ...
    private func saveDraft() async {
        guard let userId = appState.authService.currentUser?.id,
            ...
            errorMessage = "Unable to save: missing user or category"
            return
        }
        do {
    ...
    private func postTask() async {
        guard let userId = appState.authService.currentUser?.id,
              let category = selectedCategory else {
            errorMessage = "Unable to post: missing user or category"
            return
        }
        isLoading = true
        errorMessage = nil
        do {
            ...
            errorMessage = "Failed to post task: \(error.localizedDescription)"
        }
        isLoading = false
    }

    // Categories now derived from TaskCategory.allCases (single source of truth)
}
